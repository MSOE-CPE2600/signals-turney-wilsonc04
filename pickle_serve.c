/**
 * File: pickle_serve.c
 * Modified by: Caleb Wilson
 * CPE 2600 111
 * 
 * Brief summary of program: Server program for signal-based pickleball.
 * Starts as the server, sends and receives SIGUSR1 signals,
 * keeps score using pickleball rules, and ends the game at 11 points (win by 2).
 */

// Build: gcc -o p_serve pickle_serve.c
// Run:   ./p_serve <receiver_pid>

#define _POSIX_C_SOURCE 200809L
#include <stdio.h>
#include <stdlib.h>
#include <signal.h>
#include <unistd.h>
#include <time.h>

static pid_t my_pid, peer_pid, server_pid;
static int my_score = 0, opp_score = 0;
static volatile sig_atomic_t rally_active = 0;

static void rand_delay(void){ usleep(1000000 + (rand()%1000001)); }
static void bell_send(int count){
    union sigval sv; sv.sival_int = count;
    sigqueue(peer_pid, SIGUSR1, sv);
    printf("\a[SERVE %d] send (count=%d) -> %d\n",(int)my_pid,count,(int)peer_pid);
    fflush(stdout);
}
static int game_over(void){
    return ((my_score>=11 || opp_score>=11) && abs(my_score-opp_score)>=2);
}
static void announce_and_end(const char* who){
    printf("[SERVE %d] %s | FINAL: me %d - opp %d\n",
           (int)my_pid, who, my_score, opp_score);
    sigqueue(peer_pid, SIGUSR2, (union sigval){.sival_int=0});
    exit(0);
}
static void serve_first(void){ rally_active=1; rand_delay(); bell_send(1); }

static void on_gameover(int s){ (void)s;
    printf("[SERVE %d] game over signal received. FINAL: me %d - opp %d\n",
           (int)my_pid, my_score, opp_score);
    exit(0);
}

static void on_ball(int s, siginfo_t* info, void* u){ (void)s; (void)u;
    int count = info? info->si_value.sival_int : 0;
    printf("[SERVE %d] recv from %d (count=%d)  server=%d  score %d-%d\n",
           (int)my_pid, info?(int)info->si_pid:-1, count, (int)server_pid, my_score, opp_score);

    if(count==0){
        // sender missed; rally ends
        rally_active=0;
        if(server_pid==my_pid){
            my_score++;                     // only server scores
            if(game_over()) announce_and_end("I won the match");
            // keep serve
        }else{
            server_pid=my_pid;              // server lost -> switch to me
        }
        if(server_pid==my_pid) serve_first();
        return;
    }

    // exchange continues
    rand_delay();
    if(rand()%2==0) bell_send(count+1);
    else            bell_send(0);           // I miss -> opponent scores if they were server
}

int main(int argc, char** argv){
    if(argc!=2){ fprintf(stderr,"Usage: %s <receiver_pid>\n",argv[0]); return 1; }
    peer_pid=(pid_t)strtol(argv[1],NULL,10);
    if(peer_pid<=0){ fprintf(stderr,"Invalid receiver PID\n"); return 1; }

    my_pid=getpid();
    srand((unsigned)time(NULL) ^ (unsigned)my_pid);

    struct sigaction sa={0}; sigemptyset(&sa.sa_mask);
    sa.sa_flags=SA_SIGINFO; sa.sa_sigaction=on_ball;
    if(sigaction(SIGUSR1,&sa,NULL)==-1){perror("sigaction");return 1;}

    struct sigaction so={0}; so.sa_handler=on_gameover; sigemptyset(&so.sa_mask);
    if(sigaction(SIGUSR2,&so,NULL)==-1){perror("sigaction");return 1;}

    server_pid=my_pid; // I start as server
    printf("Server PID: %d, Receiver PID: %d\n",(int)my_pid,(int)peer_pid);

    serve_first(); // open the first rally
    for(;;) pause();
}
